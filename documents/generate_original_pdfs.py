...
def create_property_pdf(row, index, output_dir='documents/pdfs'):
    ...
    # Treść ogłoszenia - oryginalna treść z pliku detailed.csv
    content_header = Paragraph("📝 Opis Oferty", section_style)
    elements.append(content_header)
    
    # Użyj oryginalnej treści z kolumny description_text
    if pd.notna(row.get('description_text')) and row.get('description_text'):
        # Oryginalna treść ogłoszenia
        description_text = str(row['description_text'])
        # Zamień nowe linie na <br/>
        description_text = description_text.replace('\n', '<br/>')
        # Zamień przecinki w liczbach na spacje (dla lepszej czytelności)
        description_text = description_text.replace(',', ' ')
    else:
        # Jeśli brak oryginalnej treści, wygeneruj podstawowy opis
    ...
    # Dodaj szczegóły z pliku detailed jeśli są dostępne
    if pd.notna(row.get('floor')):
        info_data.append(['Piętro:', str(row['floor'])])
    if pd.notna(row.get('year_built')):
        info_data.append(['Rok budowy:', str(int(row['year_built']))])
    if pd.notna(row.get('building_type')):
        info_data.append(['Typ budynku:', str(row['building_type'])])
    if pd.notna(row.get('ownership_type')):
        info_data.append(['Forma własności:', str(row['ownership_type'])])
    ...
    # Dodatkowe udogodnienia (jeśli są dostępne)
    if pd.notna(row.get('equipment')) and row.get('equipment'):
        equipment_header = Paragraph("🔧 Wyposażenie", section_style)
        elements.append(equipment_header)
        
        equipment_text = str(row['equipment'])
        equipment_para = Paragraph(equipment_text, normal_style)
        elements.append(equipment_para)
        elements.append(Spacer(1, 0.5*cm))
    ...
    # Stopka - określ miasto na podstawie locality
    city = row['locality'].split()[0] if pd.notna(row.get('locality')) else "Polsce"
    footer = Paragraph(f"Wygenerowano automatycznie | Oferty mieszkań - {city}", footer_style)
...
def main(city='lodz', count=10):
    """Główna funkcja generująca PDFy
    
    Args:
        city (str): Nazwa miasta (lodz, warszawa, wroclaw)
        count (int): Liczba PDFów do wygenerowania
    """
    ...
    # Wczytaj dane z pliku detailed (zawiera oryginalną treść ogłoszeń)
    csv_path = f'/Users/mw/r_d_kurs/scraper/data/ogloszenia_{city}_detailed.csv'
    
    if not os.path.exists(csv_path):
        print(f"❌ Błąd: Plik {csv_path} nie istnieje!")
        print(f"Dostępne miasta: lodz, warszawa, wroclaw")
        return
    
    df = pd.read_csv(csv_path)
    
    print(f"\nWczytano {len(df)} ofert z CSV (plik detailed z pełnymi opisami)")
    print(f"\nGenerowanie {count} PDFów...\n")
    
    # Wybierz różnorodne oferty (różne dzielnice, różne liczby pokoi)
    # Sortuj po różnych kryteriach dla lepszej różnorodności
    sample_indices = []
    
    # Weź po kilka z różnych kategorii pokoi
    for rooms in [1, 2, 3, 4, 5]:
        room_offers = df[df['rooms'] == rooms]
        if len(room_offers) > 0:
            # Dla każdej kategorii weź kilka różnych
            sample_count = min(count // 5 + 1, len(room_offers))
            sample_indices.extend(room_offers.index[:sample_count].tolist())
        if len(sample_indices) >= count:
            break
    
    # Jeśli nie mamy wystarczająco, dodaj kolejne
    while len(sample_indices) < count and len(sample_indices) < len(df):
        idx = len(sample_indices)
        if idx < len(df) and idx not in sample_indices:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description='Skrypt do generowania ładnych PDFów z ofertami mieszkań',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog='''
Przykłady użycia:
  # Wygeneruj 10 PDFów z ofertami z Łodzi (domyślnie)
  python generate_pdfs.py
  
  # Wygeneruj 10 PDFów z ofertami z Warszawy
  python generate_pdfs.py --city warszawa
  
  # Wygeneruj 20 PDFów z ofertami z Wrocławia
  python generate_pdfs.py --city wroclaw --count 20
  
  # Skrócona forma
  python generate_pdfs.py -c lodz -n 5
        '''
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default='lodz',
        choices=['lodz', 'warszawa', 'wroclaw'],
        help='Miasto, dla którego generować PDFy (domyślnie: lodz)'
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help='Liczba PDFów do wygenerowania (domyślnie: 10)'
    )
    
    args = parser.parse_args()
    
    main(city=args.city, count=args.count)
